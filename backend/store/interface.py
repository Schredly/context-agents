...
from models import Action, AgentEvent, AgentRun, AgentUIRun, AgentUIRunEvent, ApplicationGenome, ClassificationSchema, DocGenomeExtraction, ExtractionPayload, FeedbackEvent, GenomeArtifact, GoogleDriveConfig, Integration, LLMConfig, LLMUsageEvent, ManagedIntegration, MetricsEvent, ReplitConfig, RunTelemetry, SNGenomeExtraction, ServiceNowConfig, Skill, Tenant, TenantLLMAssignment, Translation, UseCase, UseCaseRun, VideoGenomeExtraction
...
class DocGenomeExtractionStore(ABC):
    @abstractmethod
    async def create(self, extraction: "DocGenomeExtraction") -> "DocGenomeExtraction": ...
    @abstractmethod
    async def get(self, extraction_id: str) -> Optional["DocGenomeExtraction"]: ...
    @abstractmethod
    async def list_for_tenant(self, tenant_id: str) -> list["DocGenomeExtraction"]: ...
    @abstractmethod
    async def update(self, extraction_id: str, **kwargs) -> Optional["DocGenomeExtraction"]: ...
    @abstractmethod
    async def delete(self, extraction_id: str) -> bool: ...


class SNGenomeExtractionStore(ABC):
    @abstractmethod
    async def create(self, extraction: "SNGenomeExtraction") -> "SNGenomeExtraction": ...
    @abstractmethod
    async def get(self, extraction_id: str) -> Optional["SNGenomeExtraction"]: ...
    @abstractmethod
    async def list_for_tenant(self, tenant_id: str) -> list["SNGenomeExtraction"]: ...
    @abstractmethod
    async def update(self, extraction_id: str, **kwargs) -> Optional["SNGenomeExtraction"]: ...
    @abstractmethod
    async def delete(self, extraction_id: str) -> bool: ...
